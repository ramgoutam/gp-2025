...
const InventoryItems = () => {
  ...
  return <div className="space-y-4">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between px-[25px] py-[2px]">
          <div>
        ...
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                {selectedColumns.map(column => <TableHead key={column} className="bg-slate-300 hover:bg-slate-200 mx-[2px] rounded-md py-0 my-[2px] px-[14px]">
                    {column.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                  </TableHead>)}
              </TableRow>
            </TableHeader>
