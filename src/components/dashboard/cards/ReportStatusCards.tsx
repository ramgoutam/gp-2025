...
export const ReportStatusCards = () => {
  const {
    ...
    queryFn: async () => {
      ...
      const designPending = reports.filter(r => r.design_info_status === 'pending' && r.lab_script?.status === 'completed').length;
  ...
  return <div className="border rounded-lg p-6 bg-white">
      <h2 className="text-xl mb-4 text-left font-bold">Report Cards</h2>
      <div className="grid grid-cols-4 gap-4 animate-fade-in">
        {reportCards.map((card, index) => <div key={card.title} className="animate-fade-in" style={{
