...
export const LabScriptsCards = () => {
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data: scripts,
        error
      } = await supabase.from('lab_scripts').select('status');
      if (error) {
  ...
  return <div className="border rounded-lg p-6 bg-white h-full">
      <h2 className="text-xl mb-6 text-left font-bold">Lab Scripts</h2>
      <div className="space-y-8 animate-fade-in">
        <div className="grid grid-cols-4 gap-6">
          {firstRowCards.map((card, index) => <div key={card.title} className="animate-fade-in" style={{
          animationDelay: `${index * 100}ms`
        }}>
              <StatusCard title={card.title} count={card.count} icon={card.icon} color={card.color} iconColor={card.iconColor} progressColor={card.progressColor} onClick={() => {}} isActive={false} />
            </div>)}
        </div>
        <div className="grid grid-cols-3 gap-6">
          {secondRowCards.map((card, index) => <div key={card.title} className="animate-fade-in" style={{
